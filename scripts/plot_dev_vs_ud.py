...
C_A    = "#4e79a7"   # tuna-kh (blue)
C_B    = "#9467bd"   # tuna-ud (purple)
C_WIN  = "#59a14f"   # speedup > 1 (green)
C_LOSE = "#e15759"   # speedup < 1 (red)

# ── Figure: 2 rows × ND cols ─────────────────────────────────────────────────
# Row 0 — wall time box plots: label_a / label_b / KMC
# Row 1 — per-file relative speedup: label_a_wall / label_b_wall
...
for col, ds in enumerate(DATASETS):
    common = sorted(set(tuna_a[ds]) & set(tuna_b[ds]))
    ...
    ax = axes[0, col]
    bp = ax.boxplot([wa, wb], patch_artist=True,
